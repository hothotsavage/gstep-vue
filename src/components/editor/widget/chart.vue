...
const refreshChart = () => {
    ...
    vm.refreshChartData()
    // 加载流程图数据
    graph.data(vm.data.chartData) // 读取 Step 2 中的数据源到图上
    graph.render() // 渲染图
}
...
const setupChart = () => {
                ...
                if (cfg.step.title != '默认条件') {
                    group.addShape('image', {
                        attrs: {
                            ...
                            width: 14,
                            height: 14,
                            img: process.env.CLOSE_WHITE_IMAGE,
                            cursor: 'pointer',
                        },
                        // 在 G6 3.3 及之后的版本中，必须指定 name，可以是任意字符串，但需要在同一个自定义元素类型中保持唯一性
        ...
        getContent(e) {
            ...
            outDiv.innerHTML = `<div style='font-size:12px;cursor:pointer;display:flex;align-items: center;justify-content: center;'>
                                    <div style='padding: 0 5px;text-align:center;' data-tag='menu-condition'>
                                        <image src=${process.env.CONDITION_IMAGE} style="width:25px;height:25px;" data-tag='menu-condition'/>
                                    ...
                                    <div style='padding: 10px 15px;text-align:center;' data-tag='menu-audit'>
                                        <image src=${process.env.STAMP_IMAGE} style="width:25px;height:25px;" data-tag='menu-audit' />
                                        <div style='margin-top:5px;' data-tag='menu-audit'>审核人</div>
                                    </div>
                                    <div style='padding: 10px 15px;text-align:center;' data-tag='menu-notify'>
                                        <image src=${process.env.PLANE_IMAGE} style="width:25px;height:25px;" data-tag='menu-notify' />
    ...
    graph.on('delete-condition-image:click', (e) => {
        ...
        let step = item._cfg.model.step
        deleteConfirm().then(() => {
            vm.deleteChildStepById(step.id, vm.data.template.rootStep)
            refreshChart()
        })
    })
